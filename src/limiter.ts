...
import { SpamThresholds } from "./types";
...
export class SocketEventLimiter {
	...
	onLimit(
		eventName: string,
		thresholdsParams: Partial<SpamThresholds> | number,
		originalCallback: Function,
	) {
		const thresholds =
			typeof thresholdsParams === "number"
				? {
						resetInterval: thresholdsParams,
					}
				: thresholdsParams;
		// Merge custom thresholds with default values
		const spamThresholds: SpamThresholds = {
			...this.spamThresholds,
			...thresholds,
		};
		...
		const handler = (...data: any[]) => {
			...
			this.resetTimers[eventName] = setTimeout(
				resetCounter,
				spamThresholds.resetInterval,
			);
			...
			if (
				count > spamThresholds.warnLimit &&
				count <= spamThresholds.spamLimit
			) {
			...
			if (count > spamThresholds.disconnectLimit) {
				...
				const sockets = this.socket.namespace.userRoom(
					this.socket.user._id,
				).sockets;
