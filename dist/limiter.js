...
export class SocketEventLimiter {
    ...
    resetTimers;
    spamThresholds;
    constructor(socket, spamThresholds = {}, debugSpam = process.env.NODE_ENV !== "production") {
        this.debugSpam = debugSpam;
        this.socket = socket;
        this.eventCounters = {};
        this.resetTimers = {};
        this.spamThresholds = {
    ...
    onLimit(eventName, thresholdsParams, originalCallback) {
        const thresholds = typeof thresholdsParams === "number"
            ? {
        ...
        const handler = (...data) => {
            ...
            if (count > spamThresholds.warnLimit &&
            ...
            if (count > spamThresholds.disconnectLimit) {
                ...
                const sockets = this.socket.namespace.userRoom(this.socket.user._id).sockets;
